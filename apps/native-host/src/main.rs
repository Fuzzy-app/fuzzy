	...
	while let Some(body) = protocol::read_message(&mut input)? {
		let response = match serde_json::from_slice::<Request>(&body) {
			Ok(request) => match request.command.as_str() {
				"search" | "exportData" | "importData" => {
					dispatch(&mut database, &mut index_engine, request)
				}
				_ => commands::dispatch_with_file_transfers(
					&mut database,
					&mut file_transfers,
					request,
				),
...
/// コマンド名に応じて処理を振り分ける。
///
/// index機能系コマンドのみここで処理し、その他は `commands` モジュールへ委譲する。
fn dispatch(
