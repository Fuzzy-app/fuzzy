...
この文書は、Fuzzy `v0.1.1-qa.1`を別のWindows 11 PCで試すQA参加者向けです。開発ツールやコマンドは必要ありません。
...
[Fuzzy v0.1.1-qa.1 Pre-release](https://github.com/Fuzzy-app/fuzzy/releases/tag/v0.1.1-qa.1)を開き、Assetsから`Fuzzy-Setup.exe`をダウンロードします。

Releaseには次も添付します。

- `Fuzzy-0.1.1-qa.1-windows-unsigned.zip`: README、QA用成果物、チェックサムを含む一式
...
- アンインストール後も利用者の資料と書き出したバックアップが削除されない
