...
`similarity` は0.0以上1.0以下とし、`method = "exact"` の全メンバーは1.0とする。`method = "similar"`では、同じグループ内で最も近い他ファイルとのSimHash類似度を返す。同じ完全一致ペアだけを`similar`グループへ重複掲載しない。同名ファイルを識別できるよう、各メンバーにファイル名を含む `relativePath` を必ず含める。検出の暫定閾値とLSH方式は[`../重複検出方式.md`](../重複検出方式.md)を参照する。
