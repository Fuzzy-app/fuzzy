...
- セクション情報が欠けている場合、`{section}`を含むルールをコース直下までへ縮退し、その場所を保存先として提案・照合する。その他の必要情報が欠ける場合は「確認不能」を警告として返す
...
- コース名の正規化は`crates/engine-core/src/folder_names.rs`だけを正とする。TypeScript側の重複実装は削除し、生のMoodle名を送信してbackendの解決結果を利用する
